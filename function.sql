...
BEGIN
    RETURN QUERY 
    SELECT a.author_id, a.name, AVG(r.star) AS avg_rating
    FROM authors a
...
CREATE OR REPLACE FUNCTION get_avg_rating_by_genre() RETURNS TABLE (
    genre VARCHAR,
    avg_rating FLOAT
)
AS $$
BEGIN
    RETURN QUERY 
    SELECT b.genre, AVG(r.star) AS avg_rating
    FROM books b
    JOIN read r ON b.book_id = r.book_id
    GROUP BY b.genre;
END;
...
CREATE OR REPLACE FUNCTION get_book_history(p_book_id INT) RETURNS TABLE (
    event_type VARCHAR,
    user_id INT,
    user_name VARCHAR,
    date TIMESTAMP
...
BEGIN
    RETURN QUERY 
    SELECT 'reserve' AS event_type, re.user_id, u.user_name, re.reserve_date AS date
    FROM re
    JOIN users u ON re.user_id = u.user_id
    WHERE re.book_id = p_book_id
    UNION ALL
    SELECT 'read' AS event_type, r.user_id, u.user_name, r.timestamp AS date
    FROM read r
...
CREATE OR REPLACE FUNCTION get_book_recommendation(p_user_id INT) RETURNS TABLE (
    book_id INT,
    title VARCHAR,
    genre VARCHAR,
    avg_star INT
...
BEGIN
    RETURN QUERY 
    SELECT DISTINCT b.book_id, b.title, b.genre, b.avg_star
    FROM books b
    WHERE b.genre IN (
...
BEGIN
    RETURN QUERY 
    SELECT s.staff_id, s.name, COUNT(*) AS managed_books_count
    FROM staff s
...
BEGIN
    RETURN QUERY 
    SELECT u.user_id, u.user_name, COUNT(DISTINCT re.book_id) AS unique_reservations_count
    FROM users u
...
BEGIN
    RETURN QUERY 
    SELECT s.staff_id, s.name, COUNT(b.book_id) AS books_count
    FROM staff s
...
BEGIN
    RETURN QUERY 
    SELECT b.genre, COUNT(re.book_id) AS reservation_count
    FROM books b
...
BEGIN
    RETURN QUERY 
    SELECT b.book_id, b.title, COUNT(re.book_id) AS borrow_count
    FROM books b
    JOIN re ON b.book_id = re.book_id
